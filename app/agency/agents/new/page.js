...
import { ArrowLeft, Save, User, Mail, Phone, MapPin, Eye, EyeOff } from 'lucide-react'
...
export default function AddAgentPage() {
  ...
  // Check if user has agency (for agency_admin)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Get agency ID (handle both object and string)
      ...
      const registrationData = {
        ...
        agency: agencyId || undefined, // Use agency admin's agency (ensure it's a string ID)
        isActive: true, // Explicitly set to active when created by agency admin
        agentInfo: {
      ...
      // Remove empty fields
      Object.keys(registrationData.agentInfo).forEach(key => {
        if (registrationData.agentInfo[key] === undefined || registrationData.agentInfo[key] === '') {
          delete registrationData.agentInfo[key]
        }
      })

      if (!registrationData.agentInfo.licenseNumber && !registrationData.agentInfo.bio) {
        delete registrationData.agentInfo
      }
