...
import { Search, Mail, Phone, User, Calendar, Trash2, Eye, MessageSquare } from 'lucide-react'
...
export default function AgencyMessagesPage() {
  ...
  const handleView = async (message) => {
    ...
    if (!message.isRead) {
      try {
        await api.put(`/cms/contact-messages/${message._id}/read`)
        // Update local state
  ...
  // Filter messages by search term (name, email, phone, subject)
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            View all contact messages sent by users from the website
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex justify-between items-center gap-4">
            ...
            <div className="text-sm text-gray-600">
              Total Messages: <span className="font-semibold">{filteredMessages.length}</span>
            </div>
          </div>
        </div>
